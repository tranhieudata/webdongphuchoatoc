...
export default function AdminFabricPage() {
  ...
  const fetchItems = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API}/api/fabric-sample/all?limit=100`, { headers: authHeader() });
  ...
  const handleSubmit = async (e) => {
    ...
    const url = editing ? `${API}/api/fabric-sample/${editing._id}` : `${API}/api/fabric-sample/create`;
  ...
  const handleDelete = async (id) => {
    if (!confirm('Xóa mẫu vải này?')) return;
    await fetch(`${API}/api/fabric-sample/${id}`, { method: 'DELETE', headers: authHeader() });
        ...
        <div className="admin-card-header">
          <div className="admin-toolbar">
            <div className="admin-search">
              <span>🔍</span>
              <input placeholder="Tìm mẫu vải..." value={search} onChange={e => setSearch(e.target.value)} />
            </div>
          </div>
          <button onClick={openAdd} className="abtn abtn-primary">+ Thêm Mẫu Vải</button>
        </div>
        <div className="admin-card-body">
          {loading ? <div className="admin-empty"><p>Đang tải...</p></div> : (
            <table className="admin-table">
              ...
              <tbody>
                {filtered.length === 0 ? (
                  <tr><td colSpan={6}><div className="admin-empty"><span>🧵</span><p>Chưa có mẫu vải</p></div></td></tr>
                ) : filtered.map((item, i) => (
                  <tr key={item._id}>
                    <td>{i + 1}</td>
                    ...
                    <span style={{ fontSize: 12, color: '#888', marginLeft: 6 }}>{item.images?.length || 0} ảnh</span></td>
        ...
        <div className="admin-modal-backdrop" onClick={() => setModal(false)}>
          <div className="admin-modal" onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              <h3 style={{ margin: 0 }}>{editing ? 'Sửa Mẫu Vải' : 'Thêm Mẫu Vải'}</h3>
              <button onClick={() => setModal(false)} style={{ background: 'none', border: 'none', fontSize: 22, cursor: 'pointer' }}>×</button>
            </div>
            <form onSubmit={handleSubmit}>
              <div className="aform-group">
                <label>Tiêu Đề *</label>
                <input required value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} placeholder="Vải Thun Cotton 100%..." />
              </div>
              <div className="aform-group">
                <label>Mô Tả</label>
                <textarea rows={4} value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} placeholder="Mô tả chất liệu, đặc điểm..." style={{ width: '100%', padding: '8px 12px', border: '1px solid #ddd', borderRadius: 6, resize: 'vertical', fontFamily: 'inherit' }} />
              </div>
              ...
              <div className="aform-group">
                ...
                {editing && (
                  <label style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 8, cursor: 'pointer', fontSize: 13 }}>
                    ...
                    Giữ lại ảnh cũ (thêm ảnh mới vào)
                  </label>
                )}
