...
const storage = multer.diskStorage({
  ...
  filename: (req, file, cb) => cb(null, `size-${Date.now()}${path.extname(file.originalname)}`),
});
...
router.get('/', async (req, res) => {
  try {
    ...
    const items = await SizeChartModel.find({ status: 'published' })
      .sort({ order: 1, createdAt: -1 })
      .skip((page - 1) * limit)
      .limit(parseInt(limit));
    const total = await SizeChartModel.countDocuments({ status: 'published' });
...
router.get('/all', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    ...
    const items = await SizeChartModel.find(query)
      .sort({ order: 1, createdAt: -1 })
      .skip((page - 1) * limit)
      .limit(parseInt(limit));
    const total = await SizeChartModel.countDocuments(query);
    res.json({ items, total, totalPages: Math.ceil(total / limit), currentPage: Number(page) });
  } catch (err) {
    res.status(500).json({ message: 'Server error', error: err.message });
  }
...
router.get('/:slug', async (req, res) => {
  try {
    const item = await SizeChartModel.findOne({ slug: req.params.slug, status: 'published' });
...
router.post('/create', authMiddleware, adminMiddleware, upload.array('images', 10), async (req, res) => {
  try {
    ...
    const item = new SizeChartModel({ title, description, status, order: order || 0, images });
...
router.put('/:id', authMiddleware, adminMiddleware, upload.array('images', 10), async (req, res) => {
  try {
    const item = await SizeChartModel.findById(req.params.id);
    if (!item) return res.status(404).json({ message: 'Not found' });
...
export { router as SizeChartRoute };
